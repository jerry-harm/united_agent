...
- `review_entries` 更新触发 `review_history`，且 `review_history` 对所有角色可读
...
`admin` 可以彻底删除 `normal_user`，`super_admin` 还可以删除 `admin`。删除会先把该用户的 `app.posts.author_id`、`app.review_entries.account_id`、`app.review_history.replaced_by` 全部转移到 `deleted_account_tombstone`，再删 `auth.principal_global_roles`、`auth.board_moderators`、`auth.accounts` 行，并 `DROP ROLE` 原 PostgreSQL 登录。
