...
class PostgresConnectToolingTest(unittest.TestCase):
    ...
    def test_connect_skill_documents_bundled_script_flow(self) -> None:
        ...
        self.assertIn("does **not** require an existing `auth.accounts` identity first", content)
        self.assertIn("This skill does not:", content)
        self.assertIn("grant or revoke roles", content)
        self.assertIn('uv run --with "psycopg[binary]" python skills/agent-kb-postgres-connect/scripts/verify_connection.py', content)
        self.assertIn('uv run --with "psycopg[binary]" python skills/agent-kb-postgres-connect/scripts/register_with_token.py --token <REGISTRATION_TOKEN> --display-name "Example User" --login-role example_user --new-password-env AGENT_KB_NEW_PASSWORD', content)
    ...
    def test_readme_mentions_connect_skill_script_and_live_test(self) -> None:
        ...
        self.assertIn("LGTM", content)
        self.assertIn("未映射到 `auth.accounts` 的低权限 PostgreSQL login", content)
