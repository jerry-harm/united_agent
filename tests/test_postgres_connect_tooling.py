...
class PostgresConnectToolingTest(unittest.TestCase):
    ...
    def test_connect_skill_documents_bundled_script_flow(self) -> None:
        ...
        self.assertIn("thin function-caller", content)
        ...
        self.assertNotIn("read_uploaded_file.py", content)
        self.assertNotIn("uploaded_files", content)
    ...
    def test_connect_skill_bundles_local_scripts(self) -> None:
        ...
        self.assertFalse((ROOT / "skills/agent-kb-postgres-connect/scripts/upload_text_file.py").exists())
        self.assertFalse((ROOT / "skills/agent-kb-postgres-connect/scripts/read_uploaded_file.py").exists())
        self.assertFalse((ROOT / "skills/agent-kb-postgres-connect/scripts/sql/upload_text_file_insert.sql").exists())

    def test_post_and_review_flows_are_thin_function_callers(self) -> None:
        post_content = self.read_text("skills/agent-kb-postgres-connect/scripts/validate_post_flow.py")
        review_content = self.read_text("skills/agent-kb-postgres-connect/scripts/validate_review_flow.py")

        self.assertIn("from _postgres_connect_common import connect", post_content)
        self.assertIn("SELECT app.create_post(", post_content)
        self.assertNotIn("INSERT INTO app.posts", post_content)
        self.assertNotIn("uploaded_files", post_content)
        self.assertNotIn("kb://uploaded-files/", post_content)

        self.assertIn("from _postgres_connect_common import connect", review_content)
        self.assertIn("SELECT app.create_review_entry(", review_content)
        self.assertNotIn("INSERT INTO app.review_entries", review_content)
        self.assertNotIn("uploaded_files", review_content)
        self.assertNotIn("kb://uploaded-files/", review_content)
    ...
    def test_developer_guide_keeps_live_test_and_env_details(self) -> None:
        ...
        self.assertIn("thin wrapper", content)
        self.assertIn("app.create_post(...)", content)
        self.assertIn("app.create_review_entry(...)", content)
        self.assertNotIn("upload_text_file.py", content)
        self.assertNotIn("read_uploaded_file.py", content)
